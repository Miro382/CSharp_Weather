...
namespace CSharp_Weather
{
    public class CityLocator
    {
        ...
        public bool GetGeoCoordByCityName(string CityName)
        {
            try
            {
              string xml = HelpClass.GETHtml("http://nominatim.openstreetmap.org/search?city="+CityName+ "&addressdetails=1&format=xml");
        ...
        public bool GetGeoCoordByCityNameEmail(string CityName,string Email)
        {
            try
            {
                string xml = HelpClass.GETHtml("http://nominatim.openstreetmap.org/search?city=" + CityName + "&addressdetails=1&format=xml&email=" + Email);
        ...
        public bool GetGeoCoordByCityNameEmail(string CityName,string Country, string Email)
        {
            try
            {
                string xml = HelpClass.GETHtml("http://nominatim.openstreetmap.org/search?city=" + CityName + "&country="+Country + "&addressdetails=1&format=xml&email=" + Email);
        ...
        public bool GetGeoCoordByCityName(string CityName, string Country)
        {
            try
            {
                string xml = HelpClass.GETHtml("http://nominatim.openstreetmap.org/search?city=" + CityName + "&country=" + Country + "&addressdetails=1&format=xml");
